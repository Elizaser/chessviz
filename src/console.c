...
{
    if ((strlen(move) > 7) || (f(move[1]) == -1) || (f(move[4]) == -1) || (move[2] < '0') || (move[2] > '8') || (move[5] < '0') || (move[5] > '8')) {
        return 0;
    }
    return 1;
}
...
int move_q_w(char a[N][M], char move[3], int i, int j, int k, int l)
{
    if (a[i][j] == 'q') {
        if (abs(i - k) == abs(j - l) || i == k || j == l) {
            if (move[3] == '-' && a[k][l] == ' ') {
                if (((i == k || j == l) && check_place_r_q_p_k(a, i, j, k, l) == 0) || (i != k && j != l && check_place_b_q_k_p(a, i, j, k, l) == 0)) {
                    swap(&a[i][j], &a[k][l]);
                    return 0;
                }
            }
